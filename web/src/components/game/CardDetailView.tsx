...
import YugiohCard from "@/components/game/YugiohCard";
...
export default function CardDetailView({ }: CardDetailViewProps) {
  const showCardDetails = useUIStore((state) => state.showCardDetails);
  const setShowCardDetails = useUIStore((state) => state.setShowCardDetails);
  ...
  const isOpponentFaceDown = !!card && isCardAnonymous(card, viewerSide);
  ...
  const handleCloseDetailView = (event: KeyboardEvent) => {
    console.log('card detail view handler running');
  ...
  return (
    <div className={`absolute inset-0 flex items-center justify-center w-full transition-opacity duration-200 bg-black/60 ${showCardDetails ? 'opacity-100 pointer-events-auto z-50' : 'opacity-0 pointer-events-none -z-10'}`}>
      <div className={`text-white w-full h-full 3xl:w-[70%] lg:h-[70%] rounded-2xl border border-white/10 grid grid-cols-1 md:grid-cols-8 overflow-hidden relative shadow-2xl`}>
        <button
          onClick={() => setShowCardDetails(false)}
          className="absolute top-4 left-4 text-gray-400 hover:text-white text-2xl md:text-4xl font-bold z-10"
        >
          ✕
        </button>

        {/* Left Side: Details */}
        <div className="w-full h-full col-span-3 bg-gray-900 md:pt-24">
          <div className="pt-10 px-4 flex flex-col items-center justify-center">
            <h2 className="text-4xl font-bold mb-2">
              {isOpponentFaceDown ? '????' : card?.name}
            </h2>
            <div className="flex gap-2">
              <div className="text-sm text-gray-400 mb-6 uppercase tracking-wider">
                {isOpponentFaceDown ? '????' : card?.type}
              </div>
              <div className="text-sm text-gray-400 mb-6 uppercase tracking-wider font-bold" style={{ color: card?.rarity === 'common' ? 'inherit' : '#ffbb00' }}>
                {isOpponentFaceDown ? '????' : card?.rarity}
              </div>
            </div>
            <div className="grid grid-cols-4 lg:grid-cols-1">
              ...
              <div className="col-span-3 lg:grow px-2">
        ...
        <div className="flex col-span-5">
          <div className={`w-full grow flex items-center justify-center border-r border-white/10 relative bg-gray-900`}>
            {/* Independent Canvas */}
