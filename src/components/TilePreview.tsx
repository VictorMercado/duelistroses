...
export default function TilePreview({ }: TilePreviewProps) {
  ...
  const imageUrl = useMemo(() => {
    ...
    const cachedImg = getCachedTexture(selectedTile.terrain.type);
    ...
    return `/textures/${selectedTile.terrain.type}.png`;
  }, [selectedTile]);
    ...
    <div className="absolute bottom-4 left-4 w-64 h-80 bg-black/80 rounded-xl border-2 border-yellow-700 overflow-hidden shadow-2xl">
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 bg-gray-900 flex items-center justify-center relative">
            <img 
              key={selectedTile.terrain.type}
              src={imageUrl}
              alt={selectedTile.terrain.name}
